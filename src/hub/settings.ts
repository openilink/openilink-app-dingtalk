/**
 * /settings 页面 — 允许用户通过 app_token 验证身份后查看和修改钉钉配置
 */
...
export async function handleSettingsSave(
  ...
  if (dingtalkClientId) newConfig.dingtalk_client_id = dingtalkClientId;
  if (dingtalkClientSecret) newConfig.dingtalk_client_secret = dingtalkClientSecret;
  // dingtalk_robot_code 允许清空，所以特殊处理：如果表单中有该字段则始终更新
  newConfig.dingtalk_robot_code = dingtalkRobotCode;
...
function renderConfigForm(
  ...
  const maskedClientId = maskValue(cfg.dingtalk_client_id || "");
  const maskedSecret = maskValue(cfg.dingtalk_client_secret || "");
  const maskedRobotCode = maskValue(cfg.dingtalk_robot_code || "");
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>钉钉 Bridge — 修改配置</title>
  <style>${COMMON_STYLE}</style>
</head>
<body>
  <div class="card">
    <h1>钉钉 Bridge 设置</h1>
    <p class="desc">修改您的钉钉应用凭证配置</p>
    ...
    <form method="POST" action="/settings/save">
      ...
      <input name="dingtalk_client_id" placeholder="留空则不修改" />
      <p class="hint">在 <a href="https://open-dev.dingtalk.com" target="_blank">钉钉开发者后台</a> → 基础信息中获取</p>

      <label>钉钉 AppSecret</label>
      <input name="dingtalk_client_secret" type="password" placeholder="留空则不修改" />

      <label>机器人 Code</label>
      <input name="dingtalk_robot_code" placeholder="留空则清除" value="${escapeHtml(cfg.dingtalk_robot_code || "")}" />
      ...
      <div class="security-notice">
        ...
        <ul>
          ...
          <li>如需更高安全性，建议<a href="https://github.com/openilink/openilink-app-dingtalk">自行部署</a>本 App</li>
        </ul>
      </div>
