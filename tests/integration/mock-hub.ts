 ...
 * - POST /api/v1/bot/send — 记录 App 通过 HubClient 发送的消息（钉钉反向链路）
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && url.pathname === "/mock/event") {
      try {
        ...
        // 构造 HubEvent（与钉钉 webhook.ts 期望的结构一致）
        const hubEvent = {
          v: "1",
          type: "event",
    ...
    // POST /api/v1/bot/send — 记录钉钉反向链路发来的消息
    if (req.method === "POST" && url.pathname === "/api/v1/bot/send") {
